<template>
	...
	<scroll-view scroll-x="true" class="tabView" :style='{"padding":"8rpx 0","boxShadow":"0 0 12rpx rgba(255,0,0,0)","margin":"0","borderColor":"rgba(0,0,0,1)","backgroundColor":"#e2e2e2","borderRadius":"0","borderWidth":"0","width":"100%","borderStyle":"solid","height":"auto"}'>
        	<view :style='categoryName===item.caipinfenlei?sactiveItem:sitem' class="tab" v-for="(item,index) in categoryList" :key="index" :class="categoryName===item.caipinfenlei?'tabActive':''" @tap="categoryClick(item.caipinfenlei)">{{item.caipinfenlei}}</view>
        </scroll-view>
		...
		<button :style='{"boxShadow":"0 0 16rpx rgba(0,0,0,0) inset","backgroundColor":"#409EFF","borderColor":"#409EFF","borderRadius":"8rpx","color":"#fff","borderWidth":"2rpx","width":"160rpx","fontSize":"28rpx","borderStyle":"solid","height":"80rpx"}' v-if="isAuth('caipinxinxi','新增')" class="add-btn" @click="onAddTap()">新增</button>
...
<style>
	...
	.tabActive{
		...
		border-style: solid;
		border-color: rgba(0,0,0,1);
		background-color: rgba(34, 115, 191, 1);
		box-shadow: 0 0 12rpx rgba(0,0,0,.3);
	}
	.tab:last-of-type {
		border: none;
